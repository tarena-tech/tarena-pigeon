...
public class SerializationProvider {
    private static final String DEFAULT_PROVIDER = "com.tarena.dispatcher.serialization.JsonSerialization";
    private volatile static Serialization serialization;
    ...
    public static Serialization get() {
        ...
        synchronized (SerializationProvider.class) {
            ...
            Iterator<Serialization> it = loader.iterator();
            if (it.hasNext()) {
                serialization = it.next();
                return serialization;
            }

            try {
                Class<?> jsonClazz = Class.forName(DEFAULT_PROVIDER);
                serialization = (Serialization) jsonClazz.getConstructors()[0].newInstance();
                return serialization;
            } catch (Exception x) {
