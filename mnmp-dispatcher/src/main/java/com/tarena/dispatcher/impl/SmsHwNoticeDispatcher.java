...
public class SmsHwNoticeDispatcher extends AbstractNoticeDispatcher<SmsNoticeEvent> {
    private static Logger logger = LoggerFactory.getLogger(SmsHwNoticeDispatcher.class);
    ...
    private void doReceipt() {
        try {
            List<PhoneBizIdReceiptBO> bizIds = targetLogRepository.queryNotReceiptBizIds(Provider.HW_SMS);
    ...
    public void receipt() {
        ...
        ExecutorService receiptService = new ThreadPoolExecutor(1, 1, 0, TimeUnit.MICROSECONDS, new LinkedBlockingQueue<>(1),
            new BasicThreadFactory.Builder()
                .namingPattern("hw-sms-receipt-service-%d").daemon(true).build());
        receiptService.submit(new Runnable() {
            @Override public void run() {
                while (true) {
                    SmsHwNoticeDispatcher.this.doReceipt();
                }
            }
        });
    }
